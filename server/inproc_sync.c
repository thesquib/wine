...
/* Proton macOS (msync unify 2026-06-03): server-side inproc_sync backend
 * implemented over the Mach-port msync engine (server/msync.c). Ported
 * verbatim from CrossOver; the object_ops layout and get_obj_sync/
 * default_get_sync helpers are byte-identical between the two forks, so
 * this block drops in alongside proton's Linux/fsync branch (which is dead
 * on macOS). Gated by do_msync()/WINEMSYNC. */
...
void signal_inproc_sync( struct inproc_sync *sync )
{
    if (!sync->msync) return; /* create_msync failed mid-construction (OOM) */
    if (debug_level) fprintf( stderr, "set_inproc_event %d\n", sync->msync->shm_idx );
...
static void inproc_sync_destroy( struct object *obj )
{
    struct inproc_sync *sync = (struct inproc_sync *)obj;
    assert( obj->ops == &inproc_sync_ops );
    if (sync->msync) msync_destroy( sync->msync ); /* NULL when create_msync failed (OOM) */
}
...
#ifdef __APPLE__
    else if (do_msync()) reply->fsync_shm_idx = (unsigned int)fd;
#endif
