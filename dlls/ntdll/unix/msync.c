                          ...
                          int alert_obj, const LARGE_INTEGER *timeout )
{
    ...
    if (wait_any || count <= 1)
    {
        while (1)
        {
            ...
            {
                /* Local divergence from CrossOver's byte-identical msync.c:
                 * an APC may have arrived between the top-of-loop alert check
                 * and here, so re-check the alert object before reporting a
                 * timeout. Without this, our novel NtDelayExecution(TRUE,&zero)
                 * -via-msync path (the only count==0 zero-timeout caller) can
                 * miss a pending user APC and return STATUS_SUCCESS instead of
                 * honoring STATUS_USER_APC. Mirrors the pre/post-wait re-check
                 * idiom in do_single_wait(). */
